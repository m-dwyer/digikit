...
| [`14-plus-drive-format.md`](findings/14-plus-drive-format.md) | The +Drive block format: the corrected (1.16) CMD18/CMD25 and format-worker addresses, the `MmcFs` fixed-capacity project/sound/kit pools and their header/bitmap layout, and the separate real filesystem (128-byte records, extent-mapped 32 KB pages, hierarchical directories) that sample files live in, reverse engineered for `tools/plusdrive.py` |
