...
| [`04-coldfire-dsp-link.md`](findings/04-coldfire-dsp-link.md) | The periodic DSPI2 frame, the frame capture, the frame link on 1.16, the machine type reaching the SHARC, the mirror/SRC-parameter path, why DSPI2/DSPI1/FlexBus show no note-trigger or sample-data signal in a bounded run, the natural per-track kit-load-and-refresh event and why timed captures miss it (`--pre-instrs`), the DSPI2 tail past the 0x802-byte payload (real but kit-load-explained, not a trigger), the DM(0x261a10) SPI-slave driver control block, and the note-scheduling consumer `FUN_40139b78` -- why no capture has ever reached a running RTOS (a live-intro snapshot delivered PIT2/3 anyway, and the DSPI2 forcing loop re-entered its own handler, fixed) |
