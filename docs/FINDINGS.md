...
| [`04-coldfire-dsp-link.md`](findings/04-coldfire-dsp-link.md) | The periodic DSPI2 frame, the frame capture, the frame link on 1.16, the machine type reaching the SHARC, the mirror/SRC-parameter path, why DSPI2/DSPI1/FlexBus show no note-trigger or sample-data signal in a bounded run, the natural per-track kit-load-and-refresh event and why timed captures miss it (`--pre-instrs`) |
